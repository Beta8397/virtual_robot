...
public class DcMotorExDynImpl extends DcMotorDynImpl implements DcMotorEx{

    /**
     * For internal use only.
     * @param motorType
     */
    public DcMotorExDynImpl(MotorType motorType, DcMotorControllerImpl controller, int portNumber){
    ...
    public PIDFCoefficients getPIDFCoefficients(RunMode mode) {
        if (mode == RunMode.RUN_TO_POSITION)
            return new PIDFCoefficients(POS_COEFF_PROPORTIONATE, 0, 0, 0);
        else return new PIDFCoefficients(SPEED_COEFF_PROPORTIONATE, 0, 0, 0);
    }
