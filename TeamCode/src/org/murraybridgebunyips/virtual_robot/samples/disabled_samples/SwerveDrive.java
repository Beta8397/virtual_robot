package org.murraybridgebunyips.virtual_robot.samples.disabled_samples;
...
public class SwerveDrive {
    ...
    public void init(HardwareMap hardwareMap) {
        ...
        for (int i = 0; i < 4; i++) crServos[i] = hardwareMap.get(CRServo.class, crServoNames[i]);
        String[] encoderNames = new String[]{"back_left_encoder", "front_left_encoder", "front_right_encoder", "back_right_encoder"};
        for (int i = 0; i < 4; i++) encoders[i] = hardwareMap.get(DcMotor.class, encoderNames[i]);
        String[] motorNames = new String[]{"back_left_motor", "front_left_motor", "front_right_motor", "back_right_motor"};
        for (int i = 0; i < 4; i++) motors[i] = hardwareMap.get(DcMotor.class, motorNames[i]);
